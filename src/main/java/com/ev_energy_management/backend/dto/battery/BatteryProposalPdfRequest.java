 ...
 *
 * serperApiKeyNh/deepseekApiKeyNh: 매입처 실시간 검색(Serper 검색 + DeepSeek 요약)을 개인
 * 키로 한 번만 돌려보고 싶을 때 선택 입력하는 값(nh 개인 전용) — 저장/로그 절대 금지.
 * toString()도 일부러 안 만든다(record 기본 toString은 모든 필드를 그대로 찍으므로,
 * 실수로 로그에 이 객체를 찍으면 키가 그대로 남는다 - 아래 커스텀 toString으로 마스킹). */
